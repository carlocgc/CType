...
namespace Type.Objects.Enemies
{
    ...
    /// Enemy of type alpha
    /// </summary>
    public class SmallEnemyStrong : GameObject, IEnemy
    {
        ...
        public SmallEnemyStrong(Single yPos, IAccelerationProvider moveController)
        {
            _Listeners = new List<IEnemyListener>();

            _IsMoving = true;
            _IsWeaponLocked = true;
            _FireRate = TimeSpan.FromSeconds(0.5f);

            HitPoints = 4;
            Points = 75;
            CanBeRoadKilled = true;

            _Sprite = new Sprite(Game.MainCanvas, Constants.ZOrders.ENEMIES, Texture.GetTexture("Content/Graphics/Enemies/enemy5.png"))
            {
                Visible = true,
            };
