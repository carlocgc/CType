...
namespace Type.Objects.Bosses
{
    ...
    /// </summary>
    public sealed class BossStationStrong : GameObject, IEnemy, IEnemyListener
    {
        ...
        public BossStationStrong()
        {
            _Listeners = new List<IEnemyListener>();
            _Cannons = new List<BossCannon>();

            _Body = new Sprite(Game.MainCanvas, Constants.ZOrders.BOSS_BASE, Texture.GetTexture("Content/Graphics/Bosses/boss04.png")) { Visible = true, };
            Position = new Vector2(Renderer.Instance.TargetDimensions.X / 2 + _Body.Width / 2, 0);
            _Body.Offset = _Body.Size / 2;

            BossCannon rightTopMostCannon = new BossCannon(75, TimeSpan.FromMilliseconds(1300)) {Offset = new Vector2(250, 195)};
            BossCannon leftTopMostCannon = new BossCannon(75, TimeSpan.FromMilliseconds(1300)) {Offset = new Vector2(-205, 195)};
            ...
            BossCannon rightBottomMostCannon = new BossCannon(75, TimeSpan.FromMilliseconds(1300)) {Offset = new Vector2(250, -110)};
            BossCannon leftBottomMostCannon = new BossCannon(75, TimeSpan.FromMilliseconds(1300)) {Offset = new Vector2(-205, -110)};
